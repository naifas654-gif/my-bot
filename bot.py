...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == OWNER_ID:
        if text.startswith("تعديل سيرفر "):
            ...
            return
        elif "صلاحيات" in text or "سحب" in text:
            target_user = None
            ...
            if target_user:
                ...
                if "صلاحيات" in text:
                    ...
                    await update.message.reply_text(f"تم إعطاء الصلاحيات لـ {mention}")
                elif "سحب" in text:
                    distributors.discard(target_user.id)
                    await update.message.reply_text(f"تم سحب الصلاحيات من {mention}")
            else:
                await update.message.reply_text("لم أستطع تحديد العضو، يرجى الرد على رسالته أو التأكد من كتابة المنشن بشكل صحيح.")
            return
